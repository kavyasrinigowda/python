...
def main(application_properties_file, environment_properties_file, xml_files_directory, output_mode_value):
    try:
        ...
        combined_properties, missing_properties, new_properties = parse_properties_file(
            combined_properties, model.xml_files
        )
        ...
        if missing_properties:
            ...
            for missing_property in missing_properties:
                print(f" - {missing_property}")
            log_missing_properties(missing_properties)

        if new_properties:
            print("Newly added properties:")
            for prop, value in new_properties.items():
                print(f" - {prop}: {value}")

        # Output results based on mode
        if output_mode_value == 1:
            output_mode(output_mode_value, combined_properties)  # Print to console
        elif output_mode_value == 2:
            output_mode(output_mode_value, combined_properties)  # Output to properties file
